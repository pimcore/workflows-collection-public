...
- network calls newly added to test, bootstrap, build, or CI-config code WHEN the call carries data off the runner (an interpolated/dynamic URL or query string, a request body, or any env/secret/credential/file content) or fetches a remote payload that is then executed - regardless of the host. A static request to a fixed, declared endpoint that sends no runner data (e.g. a health check) is not by itself a reason to fail; judge by whether data leaves the runner or code is fetched
